...
{
	/*The questions are uploaded here*/
	...
	static String []choice= {"2 3","2","3","1","1","4","4","3","4","3","4","1","3","3","4","4","2","4","3","2","2","3","3","2","1"};
...
/* checkteststatus method */
		...
		{
			choice_button[3].setBackground(Color.green);
		}
		else
		{
			choice_button[4].setBackground(Color.lightGray);
...
class ItemHandler implements ItemListener
{
	public void itemStateChanged(ItemEvent evt)
	{
	  if(start_test)
	  {
		for(int i=0; i<boxes.length; i++) 
		{
			if(boxes[i].isSelected() ) itemCheck++;
		} 
		if(itemCheck > QuestionSeries.n[quesnum])
		{
            ...
            if(QuestionSeries.n[quesnum]==1)	
            {
            	JOptionPane.showMessageDialog(null,"<html><fontsize='4' color='00308a'><center>"+ "There is only "+QuestionSeries.n[quesnum]+" possible<br> answer to question "+(quesnum+1)+"<html>","User Information Message",JOptionPane.INFORMATION_MESSAGE);
            }
			else 
			{
